import { Actuator, ActuatorControlMode } from '@prisma/client';
...
interface IRequest {
  actuator_id: string;
  user_id: string;
  mode: ActuatorControlMode;
}

export class SetActuatorModeService {
  constructor(
    private actuatorsRepository: IActuatorsRepository,
    private environmentsRepository: IEnvironmentsRepository,
  ) {}

  public async execute({ actuator_id, user_id, mode }: IRequest): Promise<Actuator> {
    ...
    if (!environment) {
      throw new Error('User does not have permission to change this actuator mode.');
    }

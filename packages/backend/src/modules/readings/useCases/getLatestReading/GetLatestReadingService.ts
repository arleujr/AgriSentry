...
export class GetLatestReadingService {
  constructor(
  ...
  public async execute({ sensor_id, user_id }: IRequest): Promise<SensorReading> {
    ...
    const reading = await this.readingsRepository.findLatestBySensorId(sensor_id);
    if (!reading) {
      throw new Error('No readings found for this sensor.');
    }
